...
Task ID: 12
Agent: Z.ai Code (main orchestrator)
Task: FIX THE MARKS SCAN WORKFLOW — Scan Marks Sheet button not opening the workflow; finish the actual wiring (no redesign, no fake interactions).

Work Log:
- Read worklog + mapped the scan surfaces: `src/components/teacher/modules/marks/index.tsx` (mode switch), `marks/scan/*` (workspace, input-step, processing, review-grid, scan-preview, use-scan), `src/lib/marks-scan/*` (pipeline, ocr, roster-match, sheet-pdf, server), API routes `marks-entry/{route,grid,save,submit,scan-draft}`.
- ROOT CAUSE #1 (why the button "did nothing"): every authorized exam×class×subject combo for the demo teacher (Rohan Mehta) had SUBMITTED marks (UT2·10B, PA1·9A, PA1·10A Math). The Scan tab is INTENTIONALLY disabled when `grid.submitted` (exam-level lock, server-enforced in /save). The only explanation was a terse hover title → looked like a dead button.
- FIX #1 (marks/index.tsx): fuller tooltip ("Marks already submitted. Reopen/edit permission is required to change them."), aria-disabled, plus a VISIBLE amber lock line under the mode bar (hover titles are invisible on touch). Submission security NOT bypassed.
- Test data: created ExamClass + ExamSubjectConfig for Unit Test 2 × Grade 9-A × Mathematics (max 100/pass 33, 11 students, 0 marks) and later UT2 × Grade 10-A × Math (8 students) — non-submitted authorized combos for real testing.
- ROOT CAUSE #2 (deeper — geometry mismatch): generated the REAL blank sheet PDF by replicating downloadBlankMarksSheetPdf exactly (jspdf+autotable) in a bun harness, rasterized @150dpi (pdftoppm), and ported detectTable's algorithm to raw pixels (sharp). The pipeline picked the REMARKS column (153.8–195.9mm) as "marks" while teachers write in the printed MARKS column (120–154mm) — scanning a real filled sheet would always fail with "No valid marks detected".
- FIX #2 (pipeline.ts detectTable): column model now matches the actual 4-column sheet (5 vLines): roll=[L,13.2%], marks=[58.2%,76.9%] (second-to-last band), name between; generic 3-column sheets keep the old last-band model. Verified: marks cell now [120.0→153.8]mm = printed MARKS column exactly.
- ROOT CAUSE #3 (header fusion): the dark header strip (~7.5mm fill) grouped into one thick hLine whose CENTER fused the header with row 1 (OCR would see "ROLL NO."+"01" in one cell). FIX: thick runs (>16px) use the run's BOTTOM edge as the divider → row 1 = clean [62.8→74.1]mm band.
- ROOT CAUSE #4 (residual-skew fragility): a realistically tilted photo (0.3°) → deskew quantizes to 0.5° steps → 0.2° residual drifts ruling lines ~4px across the width → the strict single-row 55% ink threshold saw ZERO row lines ("Could not reliably detect the marks table" on any real photo). FIX: dilate the ink mask ±2px along the line direction (maskV for hLines, maskH for vLines) — tolerates ~0.3° residual without merging rows (rows ≥14px apart). Verified: 13 hLines + 5 vLines detected on the tilted filled sheet, 11 student bands with ink exactly where the handwritten marks are.
- FIX #5 (ocr.ts): digits whitelist '0123456789' → '0123456789AB' — the printed sheet instructs "write AB in the marks column" but the letters-only whitelist made the AB reading path (classifyMarks already handles it) unreachable. Verified: handwritten "AB" now read at 87% confidence → "Absent?" review chip.
- Browser E2E (agent-browser, teacher session): Scan tab ENABLED on non-submitted grid → workspace opens → uploaded the generated filled sheet (0.3° tilt, jittered italic digits, JPEG) → REAL Tesseract OCR (local /public/tesseract assets) → review grid "✓ 7 ready · ⚠ 2 need review · — 2 not detected" with honest per-cell confidences (e.g. 18%, 0%, 94%) → OCR misread "65" as "05" (real engine behavior) → manually corrected to 65 → confirmed 3 flagged rows (incl. AB absent) → undo/redo cycle verified → Save Draft ("Draft saved") → full page reload → draft banner → Resume review → ALL corrections persisted → Submit → validation summary exact (10 ready, 1 absent, 4 corrected, 1 blank, no blockers) → submitted.
- Canonical verification: manual Marks Entry grid now shows all 11 values read-only (87,92,78,65,71,58,43,—AB,—blank,69) with derived grades + "Submitted"; DB ExamMark rows all SUBMITTED by Rohan Mehta (Ishita status=ABSENT marks=null canonical absent; Myra PRESENT null — never zeroed); scan draft consumed (0 left); marksheet API payload includes UT2 Mathematics 87 for Aarav.
- Wrong-class safety (accidental test): uploaded the Grade 9-A sheet into the Grade 10-A workspace → sheet rolls 01-10 appear as "Extra row — not in the official roster" (NEVER created as students), submit button correctly disabled with blockers ("9 rows are not in the official roster — clear them").
- Manual entry regression: typed 88 for Kabir on UT2·Grade 10-A → autosave → "All changes saved" → DB row workflowStatus=DRAFT. Mode switching both directions works.
- Print Blank Marks Sheet: click → toast "Blank marks sheet downloaded" (doc.save() executed; the identical harness code produced the exact PDF the OCR pipeline read end-to-end — geometry loop proven: print → fill → scan → OCR → review → submit).
- Responsive sweep on the scan workspace (input + review phases): 320/360/390/414/768/1024/1440 — zero page-wide horizontal overflow; mobile [Marks Grid|Sheet Preview] tabs work; screenshots at 390px + 1440px saved (/tmp/scan-review-*.png).
- QA: `bun run lint` clean; dev.log no errors; browser console no errors.

Stage Summary:
- The scan workflow is NOW FULLY FUNCTIONAL end-to-end: button opens workspace (non-submitted grids), upload/camera/drag-drop input, real local OCR (Tesseract LSTM, vendored assets), honest per-cell confidence states, Excel-like review with keyboard nav + undo/redo, manual correction, server-scoped drafts (save/resume/discard), pre-submit validation with blockers, canonical submission through the SAME /save + /submit services as manual entry (no second DB), submission lock respected everywhere.
- 4 real bugs fixed (disabled-state messaging, marks-column geometry, header-band fusion, residual-skew line detection) + 1 wiring gap (AB whitelist). No redesign; manual entry untouched.
- Server-side authorization untouched and verified (timetable + CSA + exam config gates; FORBIDDEN paths).
- Test data added: UT2×Grade 9-A (SUBMITTED by e2e test) and UT2×Grade 10-A Math (1 DRAFT row: Kabir 88 — teacher work-in-progress state, realistic). Remaining limitation: reopening submitted marks is intentionally NOT offered to teachers (exam-office flow, per the existing business rule).

---
Task ID: teacher-audit-final
Agent: Z.ai Code (main orchestrator)
Task: Complete the Teacher Role final production-readiness audit — finish the payroll salary-change E2E (accept + decline), then sweep every teacher module, the permission matrix (4 personas), dead/duplicate systems, mock data, responsive QA (320–1440), UX consistency, and final build/QA.

Work Log:
- PAYROLL E2E — ACCEPT PATH (continued from "change request sent"): teacher Rohan Mehta's My Salary & Payments showed the pending request (₹25,000 → ₹26,000 from Oct 2026, requested by Dr. Ananya Iyer). Pre-acceptance store state captured (netBase 25000, mode simple, 1 history entry). Clicked Accept → Monthly Salary ₹26,000, Effective From 1 Oct 2026, request card cleared. Store verified: single simple-mode line (Monthly Salary 26000 — no duplicate record), history gained exactly one entry (25000→26000 by Rohan Mehta) while the original "Session salary set" entry survived, payments unchanged (Sept 25000 Pending Receipt / Aug 25000 Confirmed — history never rewritten), request status Accepted, exactly ONE salary.change_requested + ONE salary.change_accepted audit row.
- PAYROLL E2E — DECLINE PATH: principal sent a second request (₹27,000, note "Audit E2E — decline path test") from Salary & Payroll → Rohan's drawer → Edit Salary (month picker offers current + next month only — no past dates). Teacher declined with reason ("Pending confirmation of Sept payment first — please revisit next quarter."). Store verified: netBase stays 26000, effectiveFrom unchanged, history count unchanged (2 — no entry on decline), request status Declined with reason + respondedBy, exactly one salary.change_declined audit row. Principal side shows "Salary change declined · Rohan Mehta · ₹26,000 → ₹27,000" and the awaiting-approval count dropped back to 1 (Priya Nair).
- REAL BUG FOUND & FIXED — retroactive payable: after accepting a future-dated raise, the principal drawer showed Sept 2026 Payable ₹26,000 even though the raise starts Oct. Root cause: netPayableFor() used the CURRENT netBase for every period. Fix: payable for a month now resolves the LATEST history entry effective on/before that month (h.date.slice(0,7) <= periodKey), falling back to current net only with no history. Safe because the accept path always writes a history entry and every seed has the initial "Session salary set" entry. Verified after fix: staff list (Sept payable) ₹25,000, drawer Sept Payable ₹25,000/Balance ₹25,000 (pending payment settles exactly), Monthly Salary card still ₹26,000 from 1 Oct.
- ADDED — teacher Salary History section (my-salary.tsx §6): the teacher's own salary progression (initial set + every accepted change with dates, notes, actors) was invisible to the teacher after acceptance. New divide-y card section, renders ₹25,000→₹26,000 (violet transition icon) + ₹25,000 set entry; only renders when history exists.
- DASHBOARD false "all caught up" FIXED: KPI "Pending Actions" counted only needsAttention + openFollowUps while the attendance prompt below showed "not marked yet" — contradiction. Both fixed: kpi-cards includes unmarked attendance baselines in the count + context parts ("1 attendance open"), and PendingActions queue renders an amber "attendance not marked" row with the same focus-store Mark-now deep link. Verified live: Pending Actions 1 · "1 attendance open" + queue row rendered; after marking, both clear.
- CLASS ATTENDANCE verified live: Mark-now deep-link from dashboard lands focused on Grade 9-A; Mark all present + Save wrote 11 canonical PRESENT rows for 2026-09-26 (server-enforced: resolveClassScope class-teacher gate, server-derived roster, full-coverage validation, audit journal); subject-teacher view (Grade 10-A) is honestly read-only ("managed by Mrs. Kavita Sharma", no edit/save controls). Date handling verified: todayKey() is LOCAL-time (never UTC), parseDateParam builds midnight-UTC date keys, istMinutesNowClient matches the server IST boundary.
- PERMISSION MATRIX live-probed from the teacher session: subject-teacher baseline write to a non-CT class → 400 FORBIDDEN ("only the class teacher can save the daily baseline"); GET+POST /api/fees/verification → 403 (roles: PRINCIPAL, MANAGEMENT); /api/principal/academic → 403; /api/student/payments/config → 403. Nav registry verified: My Class exists ONLY when classTeacherOf.length > 0 (server-derived from /api/teacher/role); Application Reviews shows an honest empty state when no forms are assigned.
- DEAD CODE removed (verified zero importers first): src/lib/mock/attendance.ts + src/lib/mock/bus-tracking.ts (the retired mock attendance universe — only comment references remained). Confirmed /api/teacher/analytics + modules/analytics/hooks.ts|types.ts are ALIVE by design (the data layer feeding Student Growth after the Performance Analytics consolidation). All 15 teacher API routes have UI consumers.
- MOCK SWEEP: zero mock imports, zero Math.random, zero hardcoded amounts/counts in teacher components (all matches are comments describing the removed mocks). Remaining store seeds (Rohan ₹26,000 salary, fee transactions, applications) are the canonical demo-tenant dataset — consistent across modules.
- MODULE SWEEP (all real data, no redesign): My Attendance honest "staff attendance not yet configured" empty state; My Timetable real periods/exports (PDF download verified); Lesson Planner assigned-classes-only selector, real Ganita Manjari curriculum, Mark Completed 12/16→13/16 → Undo restored 12/16 (DB: 12 G9A-Math completions); Marks Entry submitted lock with explanation on UT2·9-A, draft value (Kabir 88) preserved on 10-A, scan workspace opens with scope locked; Student Directory fee records included for class-teacher class, EXCLUDED for subject-only classes ("fee records belong to the class teacher"); Student Growth single insight module with live metrics (86 avg, 4 improving PA1→UT2); My Class canonical overview + Fees tab showing the real lifecycle (collected-by-teacher → awaiting/rejected/verified states, receipts only after verification, NO verify control anywhere in the teacher UI); Communication Hub intact (11 conversations, 14 announcements, filters); Settings teacher-scoped only (no admin controls).
- POSITION-SYSTEM COPY HONESTY: accept toast claimed "permissions and Teacher Panel modules have been updated automatically" but accepting a class-less position (e.g. Examination Incharge) changes nothing visible. Fixed toast ("The responsibility is now on your record. The Principal has been notified."), banner line, and the audit-log "Permissions activated." suffix. The assignment lifecycle itself is real (assign → pending → accept/reject/clarify → audit; class scope extends when classAssigned).
- RESPONSIVE QA: zero page-wide horizontal overflow at 320/360/390/414/768/1024/1440 across Dashboard, My Salary, Class Attendance, Marks Entry, Student Directory, Student Growth, My Class, Application Reviews, Communication Hub, Settings. Announcement modal re-verified at 390 (Publish/Expires date+time pairs, no overlap, VLM 2-image pass) and 320 (modal scrolls internally; scrolled-bottom state VLM-verified: Expires inputs + Cancel/Preview fully visible, no clipping/overlap).
- CONSOLE/STALE-ERROR INVESTIGATION: one ReferenceError "requestPayrollRevision is not defined" (TeachersModule) appeared in the console buffer — traced to a STALE pre-deletion webpack chunk from this long-lived browser session (no references remain in source). Fresh reload: zero console errors. Fresh principal login → Teachers module: renders with zero console errors.
- FINAL GATES: bunx tsc --noEmit clean; bun run lint clean; dev server healthy (200, keepalive watchdog auto-restarted it once mid-session — session state survived via localStorage); dev.log no errors; browser console clean on teacher AND principal sessions.

Stage Summary:
- The salary-change workflow is the single supported payroll-change system and is verified end-to-end on both sides (accept → canonical update + history + audit + no duplicates; decline → salary unchanged + reason + audit). One real data-integrity bug fixed (future-dated raises retroactively rewriting past months' payable).
- Dashboard honesty fixed (Pending Actions now includes open attendance baselines in count + queue). Teacher salary history surfaced. Position-accept copy made honest. Two dead mock files removed.
